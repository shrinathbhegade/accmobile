function gaLogEventHandler(value) {
    cordova.plugins.firebase.analytics.logEvent("button_click", {button_text: value});
  }
  function gaReviewTherapyConversionEventHandler(value) {
    cordova.plugins.firebase.analytics.logEvent("review_therapy_conversion_event", {button_text: value});
  }
  function gaMailtoEventHandler(value) {
    cordova.plugins.firebase.analytics.logEvent("mail_to", {button_text: value});
  }
  const linkClick = (e) => {
  
    if (e != undefined) {
  
      cordova.plugins.firebase.analytics.logEvent("click", {"link_url": e.href });
...
var isNativeApplication =  !(document.URL.indexOf('localhost') === -1); //(document.URL.indexOf('http://') === -1 && document.URL.indexOf('https://') === -1);
var focusMode = false;
var orientationMode = '';
    ...
    */
    $('#Email, #Email1').click(function () {
        ...
        ko.utils.arrayForEach(appmodel.FormData.allTherapyOptions, function (item) {
            ...
            listItems += e;
            renalDose = '';
            if (item.text === appmodel.FormData.therapyOptions.Aspirin) {
                listItems += '-' + item.text + e;
                listItems += 'StdDose:' + item.emailTemplate + e;
            }
            else if (item.text === appmodel.FormData.therapyOptions.AspirinClopidogrel) {
                listItems += '-' + item.text + e;
                listItems += 'StdDose:' + item.emailTemplate + e;
            }
